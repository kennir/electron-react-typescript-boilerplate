...
export const login = actionCreator<{ username: string, password: string }>('LOGIN');
export const logout = actionCreator('LOGOUT');
export const startLoading = actionCreator('START_LOADING');
export const loginSuccess = actionCreator<{ username: string, token: string }>('LOGIN_SUCCESS');
...
const reducer = reducerWithInitialState(INITIAL_STATE).case(rehydrate, (state, {auth}) => ({
  ...
  shop_number: auth ? auth.shop_number : undefined,
})).case(logout, (state) => ({
  ...state,
  token: undefined
  ...
  ...state,
  token: undefined,
  requestStatus: { loading: false, statusCode, statusText, errorMessage }
}));
